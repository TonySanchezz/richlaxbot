...
let bot = new Flooder('196d17660dfb5e1ce50863a1d4179a33f5bd8d6190ffe45111752d25b90020764018528f0a563102c0cc7', 185402841);
function Flooder (access_token, group_id)
{
	...
	vk.updates.on('message', async (message) => {
		if(message.isUser)
		{
			...
			{
				return message.send({
					message: '[richlax|Richlax Bot], мы создали бота, что бы люди могли мстить своему врагу, уничтожив его/ее беседу.\nИли хотите пошутить над своим другом/классной беседой? Добавьте бота в чат, и вы сами увидите, что он умеет!\n\nВнимание: Бот создан в ознакомительных целях! Авторы не несет ответственность за последствия от ваших действий.'
				})
			}
			if(/^(Хочу добавить бота в беседу!)/i.test(message.text))
			{
				return message.send({
					message: 'Вы можете добавить бота в беседу.\n\nИнструкция: \n1. Зайдите в нашу [richlax|группу]\n2. Найдите кнопку "Добавить в беседу" под кнопкой ниже "Подписаться"\n3. Выберите беседу\n4. Бот сам будет спамить сразу, тогда когда вы добавили. Если не работает пишите "@richlax старт" а если не нашли кнопку "Добавить в беседу" то напишите команду "/invitebot"',
			...
			{
				message.reply('Ваша жалоба отправлена админу бота!', {
					keyboard: Keyboard.keyboard([
						Keyboard.textButton({
							label: 'Кто вы такие?'
						}),
			...
			if(/(понял|не хочу|меню рейд бота|Вернуться назад)/i.test(message.text))
			{
				return message.send({
					message: 'Слушаю тебя...',
					keyboard: Keyboard.keyboard([
			...
			if(/(Нихуя не понял!)/i.test(message.text))
			{
				return message.send('Тогда чекай закреп, просто все!:', {
					attachment: 'wall-185402841_7'
				})
			}
